...
def test(model, loader, coors, device, args):
    ...
    plt.figure(figsize=(12,18), dpi=400)
    plt.subplot(3, 2, 1)
...
def predict(model, predict_loader, coors, device, args, load_value=5.0):
    """
    进行预测并生成位移图
     """
    import os
    ...
    with torch.no_grad():
        for batch_idx, (par, u, v) in enumerate(predict_loader):
            print(f"Processing prediction batch {batch_idx + 1}")
    ...
    plt.title(f'X-Direction Displacement (Load = {load_value})')
    ...
    plt.title(f'Y-Direction Displacement (Load = {load_value})')
    plt.xlabel('X Coordinate')
    plt.ylabel('Y Coordinate')
    plt.axis('equal')
    ...
    plt.title(f'Total Displacement (Load = {load_value})')
    plt.xlabel('X Coordinate')
    plt.ylabel('Y Coordinate')
    plt.axis('equal')
