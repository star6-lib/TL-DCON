...
def predict(args, config, model, device, test_loader, coors, flag_BCxy, flag_BCy, flag_BC_load, params):
    """Predict with trained model on new boundary conditions"""
    # Load best model
    print("正在执行均匀拉伸荷载预测")
    print("输入参数形状:", test_loader.dataset.tensors[0].shape)

    try:
        model.load_state_dict(
            torch.load(r'../res/saved_models/best_model_{}_{}.pkl'.format(args.data, args.model), map_location=device))
        print("模型加载成功")
    ...
    for i, (par, u_gt, v_gt) in enumerate(test_loader):
        print(f"处理测试案例 {i + 1}")
        ...
        v_pred_np = v_pred[0].detach().cpu().numpy()

        # 计算位移幅值
        displacement_magnitude = np.sqrt(u_pred_np ** 2 + v_pred_np ** 2)

        # Combine x and y coordinates into a single array
        coordinates = np.column_stack((coor_x, coor_y))

        # Plot u displacement
        plt.figure(figsize=(12, 10))
        scatter = plt.scatter(coor_x, coor_y, c=u_pred_np, cmap='coolwarm', s=20)
        plt.title(f'Predicted u displacement - Uniform Tension 100MPa (Case {i + 1})')
        plt.colorbar(scatter, label='u displacement (mm)')
        plt.xlabel('X coordinate')
        plt.ylabel('Y coordinate')
        plt.tight_layout()
        plt.savefig(f'../res/plots/uniform_tension_case_{i + 1}_u.png', dpi=300, bbox_inches='tight')
        plt.close()

        # Plot v displacement
        plt.figure(figsize=(12, 10))
        scatter = plt.scatter(coor_x, coor_y, c=v_pred_np, cmap='coolwarm', s=20)
        plt.title(f'Predicted v displacement - Uniform Tension 100MPa (Case {i + 1})')
        plt.colorbar(scatter, label='v displacement (mm)')
        plt.xlabel('X coordinate')
        plt.ylabel('Y coordinate')
        plt.tight_layout()
        plt.savefig(f'../res/plots/uniform_tension_case_{i + 1}_v.png', dpi=300, bbox_inches='tight')
        plt.close()

        # Plot displacement magnitude
        plt.figure(figsize=(12, 10))
        scatter = plt.scatter(coor_x, coor_y, c=displacement_magnitude, cmap='viridis', s=20)
        plt.title(f'Displacement Magnitude - Uniform Tension 100MPa (Case {i + 1})')
        plt.colorbar(scatter, label='Displacement magnitude (mm)')
        plt.xlabel('X coordinate')
        plt.ylabel('Y coordinate')
        plt.tight_layout()
        plt.savefig(f'../res/plots/uniform_tension_case_{i + 1}_magnitude.png', dpi=300, bbox_inches='tight')
        plt.close()
        ...
        data_to_save = {
            ...
            'load_condition': 'uniform_tension_100MPa',
            'material_properties': {
                'youngs_modulus': params[0],
                'poissons_ratio': params[1]
            }
        }
        sio.savemat(f'../res/data/uniform_tension_case_{i + 1}.mat', data_to_save)

        print(f"Case {i + 1} 完成，结果已保存")

        # 收集所有点的数据
        for j in range(len(coor_x)):
            all_data.append([coor_x[j], coor_y[j], u_pred_np[j], v_pred_np[j], displacement_magnitude[j]])

    # 将所有点的数据保存为 Excel 文件
    df = pd.DataFrame(all_data, columns=['x', 'y', 'u', 'v', 'displacement_magnitude'])
    df.to_excel('../res/data/uniform_tension_all_points.xlsx', index=False)

    print("所有点的位移数据已保存到 Excel 文件")

    # 输出统计信息
    print("\n=== 预测结果统计 ===")
    print(f"最大u位移: {df['u'].max():.6f} mm")
    print(f"最大v位移: {df['v'].max():.6f} mm")
    print(f"最大合位移: {df['displacement_magnitude'].max():.6f} mm")
    print(f"平均合位移: {df['displacement_magnitude'].mean():.6f} mm")